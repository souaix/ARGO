...
public partial class LtCimLtEdcTestDbContext : DbContext
{
	// 建議保留無參數建構子 (給某些工具或動態建置使用)
	public LtCimLtEdcTestDbContext()
	{
	}

	// EF Core 最佳實務：在外部 (Program/Startup) 用 AddDbContext 註冊，
	// 並注入已含有連線字串的 DbContextOptions
	public LtCimLtEdcTestDbContext(DbContextOptions<LtCimLtEdcTestDbContext> options)
		: base(options)
	{
	}
	...
	//		=> optionsBuilder.UseOracle("Data Source=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=10.21.151.37)(PORT=1521))(CONNECT_DATA=(SERVICE_NAME=ltedc)));User Id=ltcimtest;Password=testcimlt0318;");
	...
	protected override void OnModelCreating(ModelBuilder modelBuilder)
	{
		...
		// 以下全部 Ignore：
		// (請依照您實際擁有的實體類別名稱，一一列出。)

		modelBuilder.Ignore<ArgoCimCimAlarmchecklog>();
		modelBuilder.Ignore<ArgoCimCimAlarmdevicedatalog>();
		modelBuilder.Ignore<ArgoCimCimScadadevicebasis>();
		modelBuilder.Ignore<ArgoCimCimScadadevicegroup>();
		modelBuilder.Ignore<ArgoCimCimScadadevicelimit>();
	


		modelBuilder.Ignore<CuMapBu2019>();
		modelBuilder.Ignore<CuMapBu2019Bin>();
		modelBuilder.Ignore<CuMapBu2019Dt>();

		modelBuilder.Ignore<EngBu2019InkBinmap>();
		modelBuilder.Ignore<EngBu2019InkBinmapDt>();


		modelBuilder.Ignore<Tbleqpequipmentbasis>();
		modelBuilder.Ignore<Tblprdproductrecipeidbasis>();
